...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		...
		while not HasModelLoaded(v.ped.model) do Citizen.Wait(1) end
		Peds[#Peds+1] = CreatePed(5, v.ped.model, v.coords.x, v.coords.y, v.coords.z-1.03, v.coords[4], false, true)
		...
		exports['qb-target']:AddCircleZone("ChairStore"..k, v.coords, 1.2, { name="ChairStore"..k, debugPoly=Config.Debug, useZ=true, }, 
		{ options = { { event = "jim-chairs:openShop", icon = "fas fa-chair", label = "Browse Store", store = Stores[v.store] }, },
			distance = 2.0
		})
	end
end)
...
--CHAIR CONTROLLER
attachedProp = 0
function attachAChair(chairModel,boneNumberSent,x,y,z,xR,yR,zR)
	removeattachedChair()
	boneNumber = boneNumberSent 
	local bone = GetPedBoneIndex(PlayerPedId(), boneNumberSent)
	RequestModel(chairModel)
	while not HasModelLoaded(chairModel) do Citizen.Wait(100) end
...
RegisterNetEvent("jim-chairs:Use", function(item)
	if not IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		QBCore.Functions.Notify('You can not sit while in a car', 'error', 7500)
	end
end)
